...
func SetupRoutes(router *gin.Engine, handler *Handler) {
	...
	router.GET("/", func(c *gin.Context) {
		c.JSON(200, gin.H{
			...
			"version": "2.0.0",
			"engines": []string{"tesseract", "easyocr", "trocr", "paddleocr", "ensemble"},
			"endpoints": []string{
				"GET  /health - Health check",
				"GET  /api/v1/model/info - Model information",
				"POST /api/v1/ocr?engine=tesseract - OCR with file upload",
				"POST /api/v1/ocr/json?engine=ensemble - OCR with JSON/base64",
				"POST /api/v1/ocr/batch?engine=easyocr - Batch OCR processing",
			},
		})
	})
